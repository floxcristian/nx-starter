...
/**
 * Valida que la especificación tenga la estructura mínima requerida
 *
 * @param spec - Especificación a validar
 * @throws {Error} Si la especificación no tiene la estructura mínima
 *
 * @example
 * ```typescript
 * validateSpecificationStructure(spec);
 * // No arroja error si la estructura es válida
 * ```
 */
export function validateSpecificationStructure(spec: SwaggerV2Document): void {
  if (!spec.info) {
    throw new Error('La especificación debe tener información básica (info)');
  }

  if (!spec.info.title) {
    throw new Error('La especificación debe tener un título');
  }

  if (!spec.info.version) {
    throw new Error('La especificación debe tener una versión');
  }

  // Validar que tenga al menos algunos paths
  if (!spec.paths || Object.keys(spec.paths).length === 0) {
    console.warn('⚠️ La especificación no tiene paths definidos');
  }
}

/**
 * Genera un resumen de la especificación final
 *
 * @param spec - Especificación final
 * @returns Objeto con estadísticas de la especificación
 *
 * @example
 * ```typescript
 * const summary = getSpecificationSummary(spec);
 * console.log(`Paths: ${summary.pathCount}, Definitions: ${summary.definitionCount}`);
 * ```
 */
export function getSpecificationSummary(spec: SwaggerV2Document): {
  pathCount: number;
  ...
  return {
    ...
    securitySchemeCount: Object.keys(spec.securityDefinitions || {}).length,
  };
}
